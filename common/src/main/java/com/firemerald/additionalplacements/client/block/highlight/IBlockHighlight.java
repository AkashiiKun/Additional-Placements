...
public interface IBlockHighlight<T extends IPlacementBlock<?>> {
    ...
    default void additionalplacements$renderHighlight(T block, PoseStack pose, VertexConsumer vertexConsumer, Player player, BlockHitResult result, Camera camera, DeltaTracker delta) {
        ...
        if (block.additionalplacements$enablePlacement(hit, player.level(), result.getDirection(), player)) {
            ...
            if (previewColor[3] > 0) additionalplacements$renderPlacementPreview(block, pose, vertexConsumer, player, result, delta, previewColor[0], previewColor[1], previewColor[2], previewColor[3]);
            pose.mulPose(DIRECTION_TRANSFORMS[result.getDirection().ordinal()]);
            float[] gridColor = APConfigs.client().gridColor();
            if (gridColor[3] > 0) additionalplacements$renderPlacementHighlight(block, pose, vertexConsumer, player, result, delta, gridColor[0], gridColor[1], gridColor[2], gridColor[3]);
            pose.popPose();
        }
    }
